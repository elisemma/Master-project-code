...
Zr_energy_list = [Zr_energy_data[foil]['energy'] for foil in Zr_energy_data if foil.startswith('Zr')]
Zr_energy_min_unc_list = [Zr_energy_data[foil]['min_unc'] for foil in Zr_energy_data if foil.startswith('Zr')]
Zr_energy_plus_unc_list = [Zr_energy_data[foil]['plus_unc'] for foil in Zr_energy_data if foil.startswith('Zr')]
...
def calc_xs(foil_name, reaction_product):
    ...
    zr_foil = Zr_foil(foil_name, reaction_product, A0, A0_unc)
    zr_foil.assign_areal_dens_w_unc_percent()
    zr_foil.assign_molar_mass()
    zr_foil.calculate_decay_constant_w_unc()
    zr_foil.assign_beam_current_w_unc()
    zr_foil.calculate_xs_w_unc()

    xs = zr_foil.calc_xs
    xs_unc = zr_foil.calc_xs_unc
    return xs, xs_unc #[mb]
