...
df_86Y_cum = pd.read_csv('../Calculated_xs/Zr_dx_86Y_ground_state_Cumulative_xs.csv')
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '39-Y-86M_empire.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path + '39-Y-86M_empire.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+0.9931*y for x,y in zip(xs_list, xs_feeding2)]
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_86Y_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 41, 86)
    E_alice_feed2, xs_alice_feed2 = get_alice_data(target_material, 40, 86)
    print(E_alice)
    print(E_alice_feed)
    print(E_alice_feed2)
    ...
    plt.errorbar(df_86Y_cum['Energy'], df_86Y_cum['xs'], xerr=[df_86Y_cum['Energy_min_unc'], df_86Y_cum['Energy_plus_unc']], yerr=df_86Y_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('86Y', 'ground_state', 39, 86, ['Zr06', 'Zr07', 'Zr08', 'Zr09'], r'$^{nat}$Zr(d,x)$^{86}$Y ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=False)