...
df_90Ym_cum = pd.read_csv('../Calculated_xs/Zr_dx_90Ym_isomeric_state_Cumulative_xs.csv')
df_90Ym_cum = df_90Ym_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp038090.tot', unpack = True, skiprows = 24)
...
def get_alice_data(target_material, Z, A, state):
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '38-Y-90M_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '38-Sr-90G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '038-090YM_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path+'038-090SrG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '038090', fileEnding_feeding)
    ...
    try:
        ...
        url_feeding1 = tend_deuteron_url(targetFoil, target, '037090', fileEnding_feeding1)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    #Get data from alice
    E_alice, xs_alice = get_alice_data(target_material, Z, A, state)
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 38, 90, 'ground_state')
    print(E_alice)
    print(E_alice_feed)
    xs_alice_list = [0]+xs_alice[0:9]+[x+y for x,y in zip(xs_alice[9:],xs_alice_feed)]
    ...
    plt.errorbar(df_90Ym_cum['Energy'], df_90Ym_cum['xs'], xerr=[df_90Ym_cum['Energy_min_unc'], df_90Ym_cum['Energy_plus_unc']], yerr=df_90Ym_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
