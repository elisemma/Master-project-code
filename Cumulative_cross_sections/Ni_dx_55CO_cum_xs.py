...
df_55Co_cum = pd.read_csv('../Calculated_xs/Ni_dx_55Co_ground_state_Cumulative_xs.csv')
df_55Co_cum = df_55Co_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp028055.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '28-Ni-55G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '28-Ni-55G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '028-055NiG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '028-055NiG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]

    if os.path.exists(path + '029-055CuG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '029-055CuG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '028055', fileEnding_feeding)
        print(url_feeding)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_55Co_cum.itertuples():
    ...
    ref_numbers = ['[45]', '[41]', '[42]', '[46]', '[47]', '[48]', '[44]', '[43]']
    ...
    plt.errorbar(df_55Co_cum['Energy'], df_55Co_cum['xs'], xerr=[df_55Co_cum['Energy_min_unc'], df_55Co_cum['Energy_plus_unc']], yerr=df_55Co_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('55CO', 'ground_state', 27, 55, ['Ni01', 'Ni02', 'Ni03', 'Ni04'], r'$^{nat}$Ni(d,x)$^{55}$Co - ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=True)
