...
df_48Sc_cum = pd.read_csv('../Calculated_xs/Ti_dx_48SC_ground_state_Cumulative_xs.csv')
df_48Sc_cum = df_48Sc_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp020048.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '20-Ca-48G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '20-Ca-48G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '020-048CaG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path+'020-048CaG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '020048', fileEnding_feeding)
    ...
    try:
        ...
        url_feeding1 = tend_deuteron_url(targetFoil, target, '019048', fileEnding_feeding1)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_48Sc_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 20, 48)
    print(E_alice)
    print(E_alice_feed)
    xs_alice_list = [0] + xs_alice[0:13] + [xs_alice[13]+xs_alice_feed[0]] + xs_alice[14:16] + [x+y for x,y in zip(xs_alice[16:],xs_alice_feed[1:])]
    print('________________')
    print(E_alice[0:13])
    print(E_alice[13])
    print(E_alice_feed[0])
    print(E_alice[14:16])
    print(E_alice[16:])
    print(E_alice_feed[1:])
    ...
    plt.errorbar(df_48Sc_cum['Energy'], df_48Sc_cum['xs'], xerr=[df_48Sc_cum['Energy_min_unc'], df_48Sc_cum['Energy_plus_unc']], yerr=df_48Sc_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
