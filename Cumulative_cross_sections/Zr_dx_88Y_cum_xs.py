...
def get_talys_data(filename, foil_name):
    ...
    E_feeding2, xs_feeding2 = np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp040088.tot', unpack = True, skiprows = 24)
    ...
    xs_list = [x+y+z for x,y,z in zip(xs_ind, xs_feeding, xs_feeding2)]
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '40-Zr-88G_empire.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path + '40-Zr-88G_empire.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '041-088NbG_coh.txt'):
        ...
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
    if os.path.exists(path + '040-088ZrG_coh.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path+'040-088ZrG_coh.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding2)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '040088', fileEnding_feeding)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_88Y_cum.itertuples():
    ...
    E_alice_feed2, xs_alice_feed2 = get_alice_data(target_material, 40, 88)
    xs_alice_list = [0]+xs_alice[0:9]+[x+y for x,y in zip(xs_alice[9:13],xs_alice_feed2[:4])]+[x+y+z for x,y,z in zip(xs_alice[13:], xs_alice_feed2[4:], xs_alice_feed)]
    ...
    plt.errorbar(df_88Y_cum['Energy'], df_88Y_cum['xs'], xerr=[df_88Y_cum['Energy_min_unc'], df_88Y_cum['Energy_plus_unc']], yerr=df_88Y_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
