...
df_95Zr_cum = pd.read_csv('../Calculated_xs/Zr_dx_95ZR_ground_state_Cumulative_xs.csv')
df_95Zr_cum = df_95Zr_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp039095.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '39-Y-95G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '39-Y-95G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '039-095YG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path+'039-095YG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '039095', fileEnding_feeding)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_95Zr_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 39, 95)
    print(E_alice)
    print(E_alice_feed)
    ...
    plt.errorbar(df_95Zr_cum['Energy'], df_95Zr_cum['xs'], xerr=[df_95Zr_cum['Energy_min_unc'], df_95Zr_cum['Energy_plus_unc']], yerr=df_95Zr_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
