...
df_60Co_cum = pd.read_csv('../Calculated_xs/Ni_dx_60Co_ground_state_Cumulative_xs.csv')
df_60Co_cum = df_60Co_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp026060.tot', unpack = True, skiprows = 24)
    E_feeding1, xs_feeding1 = np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp025060.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '26-Fe-60G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '26-Fe-60G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
        
    if os.path.exists(path + '27-Co-60M_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '27-Co-60M_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.9975*y for x,y in zip(xs_list, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '026-060FeG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '026-060FeG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]
            
    if os.path.exists(path + '027-060CoM_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '027-060CoM_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '026060', fileEnding_feeding)
        print(url_feeding)
    ...
    try:
        ...
        url_feeding1 = tend_deuteron_url(targetFoil, target, '025060', fileEnding_feeding1)
        print(url_feeding1)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_60Co_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 26, 60)
    
    print(E_alice)
    print(E_alice_feed)

    xs_alice_list = [0] + xs_alice[:8] + [x+y for x,y in zip(xs_alice[8:], xs_alice_feed)]
    ...
    plt.errorbar(df_60Co_cum['Energy'], df_60Co_cum['xs'], xerr=[df_60Co_cum['Energy_min_unc'], df_60Co_cum['Energy_plus_unc']], yerr=df_60Co_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
