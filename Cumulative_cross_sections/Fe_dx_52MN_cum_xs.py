...
df_52Mn_cum = pd.read_csv('../Calculated_xs/Fe_dx_52Mn_ground_state_Cumulative_xs.csv')
df_52Mn_cum = df_52Mn_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp027052.tot', unpack = True, skiprows = 24)
    E_feeding1, xs_feeding1= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp026052.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '26-Fe-52G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '26-Fe-52G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]

    if os.path.exists(path + '27-Co-52G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '27-Co-52G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]

    if os.path.exists(path + '25-Mn-52M_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '25-Mn-52M_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '025-052MnM_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '025-052MnM_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_ind, xs_feeding)] #Only 1.78 percent chance of IT
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '026052', fileEnding_feeding)
        print(url_feeding)
    ...
    try:
        ...
        url_feeding1 = tend_deuteron_url(targetFoil, target, '027052', fileEnding_feeding1)
        print(url_feeding)
    ...
    try:
        fileEnding_feeding2 = tendl_file_ending('00')
        url_feeding2 = tend_deuteron_url(targetFoil, target, '025052', fileEnding_feeding1)
        print(url_feeding)
        E_feeding2, xs_feeding2 = retrieve_tendl_data_from_url(url_feeding2)
        xs_tot = [x+y for x,y in zip(xs_tot, xs_feeding2)]
    except:
        print(f'No feeding for {target}')
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_52Mn_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 27, 52)
    E_alice_feed1, xs_alice_feed1 = get_alice_data(target_material, 26, 52)

    print(E_alice)
    print(E_alice_feed)
    print(E_alice_feed1)

    xs_alice_list = [0] + xs_alice[0:7] + [xs_alice[7]+xs_alice_feed1[0]] + [xs_alice[8]] + [x+y for x,y in zip(xs_alice[9:11],xs_alice_feed1[1:3])] + [xs_alice[11]] + [x+y for x,y in zip(xs_alice[12:25],xs_alice_feed1[3:16])] + [x+y+z for x,y,z in zip(xs_alice[25:], xs_alice_feed1[16:], xs_alice_feed)]

    # print(len(E_alice), len(xs_alice_list))
    # print('______')
    # print(E_alice[0:7])
    # print('______')
    # print(E_alice[7])
    # print(E_alice_feed1[0])
    # print('______')
    # print(E_alice[8])
    # print('______')
    # print(E_alice[9:11])
    # print(E_alice_feed1[1:3])
    # print('______')
    # print(E_alice[11])
    # print('______')
    # print(E_alice[12:25])
    # print(E_alice_feed1[3:16])
    # print('______')
    # print(E_alice[25:])
    # print(E_alice_feed1[16:])
    # print(E_alice_feed)
    ...
    ref_numbers = ['[56]', '[57]', '[45]', '[58]', '[59]', '[60]', '[61]']
    ...
    plt.errorbar(df_52Mn_cum['Energy'], df_52Mn_cum['xs'], xerr=[df_52Mn_cum['Energy_min_unc'], df_52Mn_cum['Energy_plus_unc']], yerr=df_52Mn_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('52MN', 'ground_state', 25, 52, ['Fe01', 'Fe02', 'Fe03', 'Fe04', 'Fe05'], r'$^{nat}$Fe(d,x)$^{52}$Mn - ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=True)
