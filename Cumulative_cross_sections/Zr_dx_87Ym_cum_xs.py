...
df_87Ym_cum = pd.read_csv('../Calculated_xs/Zr_dx_87Ym_isomeric_state_Cumulative_triks_miks.csv', comment='#')
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding = np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp040087.tot', unpack = True, skiprows = 24)
    E_feeding1, xs_feeding1 = np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp041087.tot', unpack = True, skiprows = 24)

    E_list = [0]+list(E_talys)
    xs_ind = list(xs_talys)

    xs_list = [0]+[x+y+z for x,y,z in zip(xs_ind, xs_feeding, xs_feeding1)]
            ...
            if line.strip():  # Check if line is not empty
                ...
                try:
                    if int(float(data[1])) == Z and int(float(data[2])) == A:
                        ...
                        cross_sections.append(float(data[7])) #[3] er total xs, [5] er gs, [7] er isomer 1
                except ValueError:
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '41-Nb-87G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '41-Nb-87G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]

    if os.path.exists(path + '40-Zr-87G_empire.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path + '40-Zr-87G_empire.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding2)]

    if os.path.exists(path + '41-Nb-87M_empire.txt'):
        E_feeding3, xs_feeding3 = np.loadtxt(path + '41-Nb-87M_empire.txt', unpack = True)
        xs_feeding3 = [0]+list(xs_feeding3)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding3)]

    if os.path.exists(path + '40-Zr-87M_empire.txt'):
        E_feeding4, xs_feeding4 = np.loadtxt(path + '40-Zr-87M_empire.txt', unpack = True)
        xs_feeding4 = [0]+list(xs_feeding4)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding4)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '041-087NbG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path+'041-087NbG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
        print('coh feeding 1')
    if os.path.exists(path + '040-087ZrG_coh.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path+'040-087ZrG_coh.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding2)]
        print('coh feeding 2')
    if os.path.exists(path + '041-087NbM_coh.txt'):
        E_feeding3, xs_feeding3 = np.loadtxt(path+'041-087NbM_coh.txt', unpack = True)
        xs_feeding3 = [0]+list(xs_feeding3)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding3)]
        print('coh feeding 3')
    if os.path.exists(path + '040-087ZrM_coh.txt'):
        E_feeding4, xs_feeding4 = np.loadtxt(path+'040-087ZrM_coh.txt', unpack = True)
        xs_feeding4 = [0]+list(xs_feeding4)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding4)]
        print('coh feeding 4')
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        fileEnding_feeding = tendl_file_ending(None)
        url_feeding = tend_deuteron_url(targetFoil, target, '040087', fileEnding_feeding)
        E_feeding, xs_feeding = retrieve_tendl_data_from_url(url_feeding)
        xs_tot = [x+y for x,y in zip(xs_tot, xs_feeding)]
    except:
        print(f'No feeding for {target}')

    try:
        fileEnding_feeding1 = tendl_file_ending(None)
        url_feeding1 = tend_deuteron_url(targetFoil, target, '041087', fileEnding_feeding1)
        E_feeding1, xs_feeding1 = retrieve_tendl_data_from_url(url_feeding1)
        xs_tot = [x+y for x,y in zip(xs_tot, xs_feeding1)]
    except:
        print(f'No feeding for {target}')
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_87Ym_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 41, 87)
    E_alice_feed2, xs_alice_feed2 = get_alice_data(target_material, 40, 87)
    # print(E_alice)
    # print(E_alice_feed)
    # print(E_alice_feed2)
    xs_alice_list = [0]+xs_alice[0:13]+[x+y for x,y in zip(xs_alice[13:16],xs_alice_feed2[:3])]+[x+y+z for x,y,z in zip(xs_alice[16:], xs_alice_feed2[3:], xs_alice_feed)]
    # print('______________________')
    # print(E_alice[0:9])
    # print(E_alice[9:13], E_alice_feed2[:4])
    # print(E_alice[13:], E_alice_feed2[4:], E_alice_feed)
    # print('______________________')

    # print(len(E_alice))
    # print(len(xs_alice_list))
    # xs_alice_list = [0]+xs_alice
    ...
    plt.errorbar(df_87Ym_cum['Energy'], df_87Ym_cum['xs'], xerr=[df_87Ym_cum['Energy_min_unc'], df_87Ym_cum['Energy_plus_unc']], yerr=df_87Ym_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('87Ym', 'isomeric_state', 39, 87, ['Zr01', 'Zr02', 'Zr06', 'Zr07', 'Zr08', 'Zr09', 'Zr10'], r'$^{nat}$Zr(d,x)$^{87m}$Y - ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=True)
