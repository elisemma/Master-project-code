...
df_57Co_cum = pd.read_csv('../Calculated_xs/Ni_dx_57Co_ground_state_Cumulative_xs.csv')
df_57Co_cum = df_57Co_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp029057.tot', unpack = True, skiprows = 24)
    E_feeding1, xs_feeding1 = np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp028057.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '29-Cu-57G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '29-Cu-57G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]

    if os.path.exists(path + '28-Ni-57G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '28-Ni-57G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '029-057CuG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '029-057CuG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]
    
    if os.path.exists(path + '028-057NiG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '028-057NiG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '029057', fileEnding_feeding)
        print(url_feeding)
    ...
    try:
        fileEnding_feeding = tendl_file_ending(None)
        url_feeding = tend_deuteron_url(targetFoil, target, '028057', fileEnding_feeding)
        print(url_feeding)
        E_feeding, xs_feeding = retrieve_tendl_data_from_url(url_feeding)
        xs_tot = [x+y for x,y in zip(xs_tot, xs_feeding)]
    except:
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_57Co_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 29, 57)
    E_alice_feed1, xs_alice_feed1 = get_alice_data(target_material, 28, 57)
    ...
    # xs_alice_list = [0] + xs_alice[:10] + [x+y for x,y in zip(xs_alice[10:], xs_alice_feed)]
    xs_alice_list = [0] + [x+y for x,y in zip(xs_alice[:10], xs_alice_feed1[:10])] + [x+y+z for x,y,z in zip(xs_alice[10:], xs_alice_feed, xs_alice_feed1[10:])]
    ...
    plt.errorbar(df_57Co_cum['Energy'], df_57Co_cum['xs'], xerr=[df_57Co_cum['Energy_min_unc'], df_57Co_cum['Energy_plus_unc']], yerr=df_57Co_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('57CO', 'ground_state', 27, 57, ['Ni04'], r'$^{nat}$Ni(d,x)$^{57}$Co - ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=False)
