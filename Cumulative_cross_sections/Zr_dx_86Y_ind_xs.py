...
df_86Y_gm = pd.read_csv('../Calculated_xs/Zr_dx_86Y_ground_state_Independent_xs.csv')
...
def get_talys_data(filename, foil_name):
    E_talys, xs_talys, ratio= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/'+filename, unpack = True, skiprows = 24)
    E_feeding, xs_feeding = np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp040086.tot', unpack = True, skiprows = 24)
...
def get_alice_data(target_material, Z, A, col_number):
            ...
            if line.strip():  # Check if line is not empty
                ...
                try:
                    if int(float(data[1])) == Z and int(float(data[2])) == A:
                        ...
                        cross_sections.append(float(data[col_number])) #[3] er total xs, [5] er gs, [7] er isomer 1
                except ValueError:
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '41-Nb-86G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '41-Nb-86G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
    if os.path.exists(path + '40-Zr-86G_empire.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path + '40-Zr-86G_empire.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding2)]
    if os.path.exists(path + '40-Zr-86M_empire.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path + '40-Zr-86M_empire.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding2)]
...
def get_coh_data(path, filename):
    ...
    # if os.path.exists(path + '039-086YM_coh.txt'):
    #     E_feeding, xs_feeding = np.loadtxt(path+'039-086YM_coh.txt', unpack = True)
    #     xs_feeding = [0]+list(xs_feeding)
    #     xs_list = [x+0.9931*y for x,y in zip(xs_ind, xs_feeding)]
    #     print('coh feeding')
    if os.path.exists(path + '041-086NbG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path+'041-086NbG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
        print('coh feeding 1')
    if os.path.exists(path + '040-086ZrG_coh.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path+'040-086ZrG_coh.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding2)]
        print('coh feeding 2')
    if os.path.exists(path + '040-086ZrM_coh.txt'):
        E_feeding2, xs_feeding2 = np.loadtxt(path+'040-086ZrM_coh.txt', unpack = True)
        xs_feeding2 = [0]+list(xs_feeding2)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding2)]
        print('coh feeding 3')
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '040086', fileEnding_feeding)
    ...
    try:
        ...
        url_feeding1 = tend_deuteron_url(targetFoil, target, '041086', fileEnding_feeding1)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_86Y_gm.itertuples():
    ...
    #Get data from alice
    E_alice, xs_alice = get_alice_data(target_material, Z, A, 5)
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 41, 86, 3)
    E_alice_feed2, xs_alice_feed2 = get_alice_data(target_material, 40, 86, 3)
    print(E_alice)
    print(E_alice_feed)
    print(E_alice_feed2)
    ...
    plt.errorbar(df_86Y_gm['Energy'], df_86Y_gm['xs'], xerr=[df_86Y_gm['Energy_min_unc'], df_86Y_gm['Energy_plus_unc']], yerr=df_86Y_gm['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('86Y', 'ground_state', 39, 86, ['Zr06', 'Zr07', 'Zr08', 'Zr09'], r'$^{nat}$Zr(d,x)$^{86}$Y - ', 'Independent', save_fig=True, write_csv=False, exfor_manuel=True)
