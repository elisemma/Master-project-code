...
df_52Mn_cum = pd.read_csv('../Calculated_xs/Ni_dx_52Mn_ground_state_Cumulative_xs.csv')
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp026052.tot', unpack = True, skiprows = 24)
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '026-052FeG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '026-052FeG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)] 
    
    if os.path.exists(path + '027-052CoG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '027-052CoG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]

    if os.path.exists(path + '028-052NiG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '028-052NiG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        fileEnding_feeding1 = tendl_file_ending('00')
        url_feeding1 = tend_deuteron_url(targetFoil, target, '025052', fileEnding_feeding1)
        print(url_feeding1)
        E_feeding1, xs_feeding1 = retrieve_tendl_data_from_url(url_feeding1)
        xs_tot = [x+0.0178*y for x,y in zip(xs_tot, xs_feeding1)]
    except:
        print(f'No feeding1 for {target}')
...
plot_xs('52MN', 'ground_state', 25, 52, ['Ni01', 'Ni02'], r'$^{nat}$Ni(d,x)$^{52}$Mn - ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=True)
