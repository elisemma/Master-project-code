...
df_56Co_cum = pd.read_csv('../Calculated_xs/Ni_dx_56Co_ground_state_Cumulative_xs.csv')
df_56Co_cum = df_56Co_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp028056.tot', unpack = True, skiprows = 24)
    E_feeding1, xs_feeding1 = np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp029056.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '28-Ni-56G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '28-Ni-56G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]

    if os.path.exists(path + '29-Cu-56G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '29-Cu-56G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '028-056NiG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '028-056NiG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]

    if os.path.exists(path + '029-056CuG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '029-056CuG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.0178*y for x,y in zip(xs_list, xs_feeding)]
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '028056', fileEnding_feeding)
        print(url_feeding)
    ...
    try:
        ...
        url_feeding1 = tend_deuteron_url(targetFoil, target, '029056', fileEnding_feeding1)
        print(url_feeding)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_56Co_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 28, 56)
    ...
    plt.errorbar(df_56Co_cum['Energy'], df_56Co_cum['xs'], xerr=[df_56Co_cum['Energy_min_unc'], df_56Co_cum['Energy_plus_unc']], yerr=df_56Co_cum['xs_unc'], marker='D', markersize=5, linewidth=3, linestyle='', color='hotpink', label='This work')
