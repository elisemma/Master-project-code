...
df_89Zr_cum = pd.read_csv('../Calculated_xs/Zr_dx_89ZR_ground_state_Cumulative_xs.csv')
df_89Zr_cum = df_89Zr_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp041089.tot', unpack = True, skiprows = 24)
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '41-Nb-89G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '41-Nb-89G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]

    if os.path.exists(path + '40-Zr-89M_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '40-Zr-89M_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.9377*y for x,y in zip(xs_ind, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '041-089NbG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path+'041-089NbG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
    
    if os.path.exists(path + '040-089ZrM_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path+'040-089ZrM_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+0.9377*y for x,y in zip(xs_ind, xs_feeding)]
    return E_list, xs_list
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding1 = tend_deuteron_url(targetFoil, target, '041089', fileEnding_feeding1)
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_89Zr_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 41, 89)
    print(E_alice)
    print(E_alice_feed)
    xs_alice_list = [0]+xs_alice[0:5]+[x+y for x,y in zip(xs_alice[5:],xs_alice_feed)]
    ...
    plt.errorbar(df_89Zr_cum['Energy'], df_89Zr_cum['xs'], xerr=[df_89Zr_cum['Energy_min_unc'], df_89Zr_cum['Energy_plus_unc']], yerr=df_89Zr_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('89ZR', 'ground_state', 40, 89, ['Zr01', 'Zr02', 'Zr03', 'Zr04', 'Zr06', 'Zr07', 'Zr08', 'Zr09', 'Zr10'], r'$^{nat}$Zr(d,x)$^{89}$Zr - ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=True)
