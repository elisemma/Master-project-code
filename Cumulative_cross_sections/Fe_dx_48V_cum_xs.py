...
df_48V_cum = pd.read_csv('../Calculated_xs/Fe_dx_48V_ground_state_Cumulative_xs.csv')
df_48V_cum = df_48V_cum[::-1]
...
def get_talys_data(filename, foil_name):
    ...
    # E_feeding, xs_feeding= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp026048.tot', unpack = True, skiprows = 24)
    # E_feeding1, xs_feeding1= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp025048.tot', unpack = True, skiprows = 24)
    E_feeding2, xs_feeding2= np.loadtxt(f'../talys_calculations/talys_{foil_name[0:2]}/talys/rp024048.tot', unpack = True, skiprows = 24)
    ...
    xs_list = [x+y for x,y in zip(xs_ind,xs_feeding2)]
...
def get_empire_data(path, filename):
    ...
    if os.path.exists(path + '26-Fe-48G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '26-Fe-48G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]

    if os.path.exists(path + '25-Mn-48G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '25-Mn-48G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]

    if os.path.exists(path + '24-Cr-48G_empire.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '24-Cr-48G_empire.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]
...
def get_coh_data(path, filename):
    ...
    if os.path.exists(path + '026-048FeG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '026-048FeG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_ind, xs_feeding)]
        print('coh feeding')

    if os.path.exists(path + '025-048MnG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '025-048MnG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]
        print('coh feeding')


    if os.path.exists(path + '024-048CrG_coh.txt'):
        E_feeding, xs_feeding = np.loadtxt(path + '024-048CrG_coh.txt', unpack = True)
        xs_feeding = [0]+list(xs_feeding)
        xs_list = [x+y for x,y in zip(xs_list, xs_feeding)]
        print('coh feeding')
...
def get_tendl_data(targetFoil, target, product, isomerLevel):
    ...
    try:
        ...
        url_feeding = tend_deuteron_url(targetFoil, target, '024048', fileEnding_feeding)
        print(url_feeding)
        E_feeding, xs_feeding = retrieve_tendl_data_from_url(url_feeding)
        xs_tot = [x+y for x,y in zip(xs, xs_feeding)]
    except:
        print(f'No feeding for {target} (Tendl)')

    try:
        fileEnding_feeding1 = tendl_file_ending(None)
        url_feeding1 = tend_deuteron_url(targetFoil, target, '025048', fileEnding_feeding1)
        print(url_feeding1)
        E_feeding1, xs_feeding1 = retrieve_tendl_data_from_url(url_feeding1)
        xs_tot = [x+y for x,y in zip(xs, xs_feeding1)]
    except:
        print(f'No feeding1 for {target} (Tendl)')

    try:
        fileEnding_feeding2 = tendl_file_ending(None)
        url_feeding2 = tend_deuteron_url(targetFoil, target, '026048', fileEnding_feeding2)
        print(url_feeding2)
        E_feeding2, xs_feeding2 = retrieve_tendl_data_from_url(url_feeding2)
        xs_tot = [x+y for x,y in zip(xs, xs_feeding2)]
    except:
        print(f'No feeding2 for {target} (Tendl)')
...
def plot_xs(reaction_product, state, Z, A, foil_list, title, xs_type, exfor_manuel=False, write_csv=False, save_fig=False, use_R_to_calc_xs=True):
    ...
    if write_csv == True:
        ...
        with open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for row in df_48V_cum.itertuples():
    ...
    E_alice = [0]+E_alice
    E_alice_feed, xs_alice_feed = get_alice_data(target_material, 24, 48)

    print(E_alice)
    print(E_alice_feed)

    xs_alice_list = [0] + xs_alice[0:16] + [x+y for x,y in zip(xs_alice[16:],xs_alice_feed)]
    ...
    plt.errorbar(df_48V_cum['Energy'], df_48V_cum['xs'], xerr=[df_48V_cum['Energy_min_unc'], df_48V_cum['Energy_plus_unc']], yerr=df_48V_cum['xs_unc'], marker='D', markersize=4, linewidth=2, linestyle='', color='hotpink', label='This work')
...
plot_xs('48V', 'ground_state', 23, 48, ['Fe01', 'Fe02', 'Fe03', 'Fe04', 'Fe05'], r'$^{nat}$Fe(d,x)$^{48}$V - ', 'Cumulative', save_fig=True, write_csv=False, exfor_manuel=True)
